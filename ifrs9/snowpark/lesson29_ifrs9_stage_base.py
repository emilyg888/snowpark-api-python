...
def build_ifrs9_stage_base(session: Session, reporting_date: str = "2026-05-31"):
    loans = session.table("IFRS9.LOAN_ACCOUNT")

    return (
        loans.with_column("AS_OF_DATE", dateadd("day", col("AS_OF_DT"), lit("1960-01-01")))
        ...
        .select(
            ...
            col("DPD"),
            col("CURR_RISK_GRADE"),
            when((col("DEFAULT_FLG") == lit("Y")) | (col("DPD") >= lit(90)), lit("3"))
            .when(
...
def main(session: Session):
    ...
    stage_base.write.mode("overwrite").save_as_table("IFRS9.STAGE_RESULT_SP")
